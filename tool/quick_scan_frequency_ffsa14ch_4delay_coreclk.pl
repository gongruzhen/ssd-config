...
my $fbdiv_limit2 = ord("b");
#my $fbdiv_limit = sprintf("%x", $f_cha);
my $channel_count=14;
my $lun_per_channel=16;
#print "core3";
  open MyFile ,">quick_scan_frequency_ffsa_coreclk.txt";

my $process1_enable=1;
...
#print "core5";
print "$core_limit";
for (my $core_numerator=$core_start; $core_numerator<=$core_limit; $core_numerator++) {
    my $core_numerator_cha = sprintf("%c", $core_numerator);
    my $core_pdll=$zero.$core_denominator."0".$core_numerator_cha."00";
    my $core_tdll=$zero.$core_denominator."0".$core_numerator_cha."01";
        #my $realfreq=($fbdiv+1)/($pdiv+1)*100;
        my $core_reg_addr = 242;
        my $core_reg_addr_hex = sprintf("%x", $core_reg_addr);
        system('./ztool','utils', 'poke-regs', $core_reg_addr_hex, $core_pdll);
        system('./ztool','utils', 'poke-regs', $core_reg_addr_hex, $core_tdll);
        system('./ztool','utils', 'poke-regs', $core_reg_addr_hex, $core_pdll);

    print "core clk frequence at address $core_reg_addr_hex: $core_tdll";
    print "\n";
    print "core clk frequence at address $core_reg_addr_hex: $core_tdll";
    print "\n";
    print "core clk frequence at address $core_reg_addr_hex: $core_tdll";
...
#open MyFile ,">n_dci_s_dqs_clk$pdll.csv";
    ...
    print MyFile "core clk frequence address $core_reg_addr_hex: $core_tdll";
    print MyFile "\n";
    print MyFile "core clk frequence address $core_reg_addr_hex: $core_tdll";
    print MyFile "\n";
    print MyFile "core clk frequence address $core_reg_addr_hex: $core_tdll";
    ...
    print MyFile "core clk frequence address $core_reg_addr_hex: $core_tdll";
    print MyFile "\n";
#print "core_loop $core_numerator";
...
for (my $delay=0; $delay<=$delay_num; $delay++) {
    my $delay_hex = sprintf("%x",$delay*60/$delay_num);
    print "\td$delay-h$delay_hex";
    print MyFile "\td$delay-h$delay_hex";
#print "core_loop3 $delay";
}
for (my $delay=0; $delay<=$delay_num; $delay++) {
    my $delay_hex = sprintf("%x",$delay*60/$delay_num);
    print "\td$delay-h$delay_hex";
...
print "\n";
print MyFile "\n";
for (my $channel=13; $channel<$channel_count; $channel++) {
    print "ECC";
    print MyFile "ECC";
    for (my $delay=0; $delay<=$delay_num; $delay++) {
		for (my $dly_channel=0; $dly_channel<$channel_count; $dly_channel++) {
			my $reg_addr = 136 + $dly_channel*4 + 1;
			my $reg_addr_hex = sprintf("%x", $reg_addr);
			my $value = $delay * 60 / $delay_num;
