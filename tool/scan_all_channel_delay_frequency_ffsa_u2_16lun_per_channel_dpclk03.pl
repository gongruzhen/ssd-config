...
my $delay_start=0;
open MyFile ,">scan_all_channel_delay_frequency_ffsa_u2_16lun_per_channel_dpclk03.txt";
...
for (my $channel=0; $channel<$channel_count; $channel++) {
	...
	my $lun_even = $min_lun.",$min_lun+2".",$min_lun+4".",$min_lun+6".",$min_lun+8".",$min_lun+10".",$min_lun+12".",$min_lun+14";
	my $ecc_count;
...
#	open (my $fh, "-|", "./ztool ifmode 0 1 -fo -T phylun:$lun_even");
	while (<$fh>) {
